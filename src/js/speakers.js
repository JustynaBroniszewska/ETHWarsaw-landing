...
const renderSpeaker = (speaker, parent) => {
  ...
  speakerItem.innerHTML = `
    <img class="speaker__image" src="${speaker.image ? speaker.image : '/public/images/speaker_placeholder.svg'}" alt="${speaker.name ? speaker.name : ''} image" loading="lazy" decoding="async"/>
...
const handleSpeakersFetch = () => {
  ...
  fetch(`/public/documents/speakers.json`)
    .then((response) => {
      if (response.ok) {
        return response
      }
